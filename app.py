...
def check(email):
    url = " https://niz4ik299c.execute-api.us-east-1.amazonaws.com/plasma/getdata?email=" + email
...
def register():
    ...
    if 'errorType' in check(x[1]):
        url = " https://niz4ik299c.execute-api.us-east-1.amazonaws.com/plasma/registration?" + params
...
@app.route('/stats')
def stats():
    url = " https://niz4ik299c.execute-api.us-east-1.amazonaws.com/plasma/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    # print(bloodgrp)
    url = " https://niz4ik299c.execute-api.us-east-1.amazonaws.com/plasma/requestonbloodgroups?blood=" + bloodgrp
    status = requests.request("GET", url)
    a = status.json()
    print(a)
    phone = []
    for i in a:
        url = "https://www.fast2sms.com/dev/bulk?authorization=vj4lQY5ahKIqRXn1JrZG6NCoPVgDb8wfAdMUuTOSWkc9By0eL7kT2bM49lReZordcFuUNBKhDPjEgfO5&sender_id=FSTSMS&message=need plasma of your blood group&language=english&route=p&numbers="+str(i['phone'])
